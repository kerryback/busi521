...
\newcommand{\tu}{\tilde{u}}
\begin{document}
\title{\vskip 0.5in Day 16}
\subtitle{Dynamic Models}
...
\bfr\frametitle{Returns}
Dates $t=0,1,2,\ldots$.  No tildes anymore for random things.  Information grows over time as random variables are observed.
...
Risk-free return from $t$ to $t+1$ is $R_{f,t+1}$.
\end{frame}

\begin{frame}{SDFs}
  

Denote SDF at date 0 for pricing payoffs that occur at date $t$ by $M_t$.

This means that the date--0 price of $X_t$ paid at $t$ is $\mye[M_tX_t]$.  And, $M_0=1$.

With no uncertainty, or with risk neutrality,
$$M_t = \frac{1}{R_{f,1}} \times \frac{1}{R_{f,2}} \times \cdots \times \frac{1}{R_{f,t}}$$

\end{frame}

\begin{frame}{Pricing at Later Dates}

What is the value at date $s$ of a payoff $X_t$ at $t>s$?  

It must be
$$\mye_s \left[\frac{M_t}{M_s}X_t\right]$$
where $E_s$ denotes expectation conditional on date $s$ information.  Call this $X_s$ and multiply by $M_s$ to get
$$M_sX_s = \mye_s[M_tX_t]$$
Thus, SDF $\times$ Value = martingale (Value = value of asset that pays no dividends, or dividends are reinvested.)
\end{frame}


\begin{frame}{No Uncertainty or Risk Neutrality}
 \begin{align*}
M_s &= \frac{1}{R_{f,1}} \times \cdots \times \frac{1}{R_{f,s}}\\
M_t &= \frac{1}{R_{f,1}} \times \cdots \times \frac{1}{R_{f,s}} \times \frac{1}{R_{f,s+1}} \times \cdots \times \frac{1}{R_{f,t}}
 \end{align*}  
 
So,
  $$\frac{M_t}{M_s} = \frac{1}{R_{f,s+1}}\times \cdots \times \frac{1}{R_{f,t}}$$
...
$$\mye_t\left[\frac{\delta u'(C_{t+1})}{u'(C_t)}R_{i,t+1}\right] = 1$$
 This is called the Euler equation.  It is derived by the same logic as in a single-period model.

 The Euler equation is equivalent to: 
$$M_t := \frac{\delta^t u'(C_t)}{u'(C_0)}$$
is an SDF process.
The one-period SDFs are
...
Let $C$ denote aggregate consumption.    Assume there is a representative investor with CRRA utility and risk aversion $\rho$.  Then, the one-period SDF is
...
Define the market portfolio as the claim to future consumption.  Consumption is the dividend of the portfolio.  Assume consumption growth $C_{t+1}/C_t$ is iid lognormal.
